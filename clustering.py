...
parser.add_option("-d", "--distances", dest='distances', default='phashes-diffs.json',
                  help="file with pairwise distances")
...
parser.add_option("-o", "--output", dest='output', default='clustering_output.txt',
                  help="file that includes clustering output")
...
def print_clusters_to_file(clustering_output, filename):
    ...
    for k in range(-1, num_clusters):
        ...
        if len(indices) > 0:
            ...
            if k != -1:
                medroid, medroid_path = find_cluster_medroid_phash(clustering_output, k, phashes_dict, distance_matrix,
                                                                   index_image)
...
clustering = DBSCAN(eps=CLUSTERING_THRESHOLD, metric='precomputed', n_jobs=8, min_samples=CLUSTERING_MIN_SAMPLES).fit(
    distance_matrix.tocsr())
