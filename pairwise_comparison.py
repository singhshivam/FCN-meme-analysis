        ...
        for _ in range(len_hashes - 1):
            ...
            for batch in range(i + 1, len_hashes, FLAGS.batch_size):
                diff = sess.run(diff_op,
                                feed_dict={hash_i: hashes[i], hashes_j: hashes[batch:batch + FLAGS.batch_size]})
            pbar.update(i)
...
def seek_queue_pair(hashes, outdir):
    ...
    sess, hashes, enqueue_pair_op, init_i, batch_size, queue_hash_i, queue_hash_j)) for init_i in
                       range(last_index, len_hashes, batch_size)]
...
def op_runner(sess, hashes, dequeue_op, init_i, batch_size, diff_op_many, diff_hash_i, diff_hashes_j, pbar,
              num_devices=1):
    len_hashes = len(hashes)
...
def seek_queue_many(ids, hashes, outdir, blacklist, hashes_diff):
    ...
    enqueue_threads = [threading.Thread(target=check_batch_many,
                                        args=[sess, hashes, enqueue_op, init_i, batch_size, queue_i, queue_hash_i,
                                              blacklist]) for init_i in range(last_index, len_hashes, batch_size)]
    # Start the threads and wait for all of them to stop.
    for t in enqueue_threads:
        t.isDaemon()
        t.start()
...
def seek_queue_many_device(ids, hashes, outdir, blacklist, hashes_diff, devices, device):
    ...
    enqueue_threads = [threading.Thread(target=check_batch_many,
                                        args=[sess, hashes, enqueue_op, init_i, batch_size, queue_i, queue_hash_i,
                                              blacklist, num_devices]) for init_i in
                       range(last_index, len_hashes, batch_size)]
    # Start the threads and wait for all of them to stop.
    for t in enqueue_threads:
        t.isDaemon()
    ...
    for _ in range(devices.index(device), len_hashes - 1 - len(blacklist), num_devices):
        ...
        i, hash_i = sess.run(dequeue_op)
        diff, filter, where = sess.run([diff_op_many, filter_op, where_op],
                                       feed_dict={diff_hash_i: hash_i, diff_hashes_j: hashes[i + 1:]})
        for j in where:
            j_rel = j[0]
            j_abs = i + j_rel + 1
            key_id = ids[i] + '-' + ids[j_abs]
